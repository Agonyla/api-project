...
import static com.agony.project.constant.UserConstant.ADMIN_ROLE;
import static com.agony.project.constant.UserConstant.USER_LOGIN_STATE;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
     ...
     * 盐值，混淆密码
     */
    public static final String SALT = "Agony";
    ...
    public User userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        if (userAccount.length() < 4) {
            throw new BusinessException(ReturnCodeEnum.PARAMS_ERROR, "账号错误");
        }
        if (userPassword.length() < 8) {
            throw new BusinessException(ReturnCodeEnum.PARAMS_ERROR, "密码错误");
        }
        // 2. 加密
        String encryptPassword = DigestUtils.md5DigestAsHex((SALT + userPassword).getBytes());
        // 查询用户是否存在
        ...
        if (user == null) {
            ...
            throw new BusinessException(ReturnCodeEnum.PARAMS_ERROR, "用户不存在或密码错误");
        }
        // 3. 记录用户的登录态
    ...
    public boolean userLogout(HttpServletRequest request) {
        if (request.getSession().getAttribute(USER_LOGIN_STATE) == null) {
            throw new BusinessException(ReturnCodeEnum.OPERATION_ERROR, "未登录");
        }
        // 移除登录态
    ...
    public User getLoginUser(HttpServletRequest request) {
        ...
        if (currentUser == null || currentUser.getId() == null) {
            throw new BusinessException(ReturnCodeEnum.NOT_LOGIN_ERROR);
        }
